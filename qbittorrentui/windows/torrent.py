...
from qbittorrentui.config import MAX_TORRENT_CONTENT_FILENAME_WIDTH
...
class TorrentWindow(uw.Columns):
    ...
    def __init__(self, main, torrent_hash, torrent, client):
        ...
        self.tabs = {"General": TorrentWindow.GeneralDisplay(),
                     "Trackers": TorrentWindow.TrackersDisplay(),
                     "Peers": TorrentWindow.PeersDisplay(),
                     "Content": TorrentWindow.ContentDisplay(client, torrent_hash),
                     }

        self.tabs_column_w = TorrentWindow.TorrentTabsDisplay(list(self.tabs.keys()))
    ...
    class TorrentTabsDisplay(uw.ListBox):
        def __init__(self, tabs: list):
            ...
            super(TorrentWindow.TorrentTabsDisplay, self).__init__(self.list_walker)

            self.__selected_tab_pos = None

        def keypress(self, size, key):
            log_keypress(logger, self, key)
            key = super(TorrentWindow.TorrentTabsDisplay, self).keypress(size, key)
    ...
    class GeneralDisplay(uw.ListBox):
        def __init__(self):
            ...
            def create_widgets():
                val_cont = TorrentWindow.GeneralDisplay.TorrentWindowGeneralTabValueContainer
            ...
            super(TorrentWindow.GeneralDisplay, self).__init__(walker)
        ...
        def keypress(self, size, key):
            log_keypress(logger, self, key)
            key = super(TorrentWindow.GeneralDisplay, self).keypress(size, key)
            return key
        ...
        class TorrentWindowGeneralTabValueContainer(uw.Columns):
            def __init__(self, data_elements: list, caption: str, format_func, source: str = "properties"):
                super(TorrentWindow.GeneralDisplay.TorrentWindowGeneralTabValueContainer, self).__init__([],
                                                                                                         dividechars=1,
                                                                                                         focus_column=None,
                                                                                                         min_width=1,
                                                                                                         box_columns=None)
    ...
    class TrackersDisplay(uw.ListBox):
        def __init__(self):
            ...
            super(TorrentWindow.TrackersDisplay, self).__init__(self.walker)
        ...
        def keypress(self, size, key):
            log_keypress(logger, self, key)
            key = super(TorrentWindow.TrackersDisplay, self).keypress(size, key)
            return key

    class PeersDisplay(uw.ListBox):
        def __init__(self):
            self.walker = uw.SimpleFocusListWalker([])
            super(TorrentWindow.PeersDisplay, self).__init__(self.walker)
        ...
        def keypress(self, size, key):
            log_keypress(logger, self, key)
            key = super(TorrentWindow.PeersDisplay, self).keypress(size, key)
            return key
    ...
    class ContentDisplay(uw.Pile):
        ...
        def __init__(self, client: Connector, torrent_hash):
            ...
            self.focused_node_class = TorrentWindow.ContentDisplay.DirectoryNode
            self.collapsed_dirs = []
            ...
            super(TorrentWindow.ContentDisplay, self).__init__(w_list)

        def update(self, sender, **kw):
            start_time = time()
            torrent_content = kw.get('content', [])

            content = TorrentWindow.ContentDisplay.Content(client=self.client,
        ...
        def keypress(self, size, key):
            log_keypress(logger, self, key)
            key = super(TorrentWindow.ContentDisplay, self).keypress(size, key)
            return key

        class Content(object):
            def __init__(self, client, torrent_hash, content, collapsed_dirs: list):
                super(TorrentWindow.ContentDisplay.Content, self).__init__()
                self.client = client
                self.torrent_hash = torrent_hash
            ...
            def _add_node_or_leaf(self, content_list: list, name: str, content: dict):
                if self.dir_sep() in name:
                    ...
                    if index is not None:
                        self._add_node_or_leaf(content_list=content_list[index]['children'],
                                               name=children_name,
                                               content=content)
                else:
        ...
        class FlagFileWidget(uw.TreeWidget):
            ...
            def load_inner_widget(self):
                ...
                is_dir = self.get_node().content.is_dir(path)  # type(self.get_node()) == TorrentWindow.ContentWindow.DirectoryNode
                ...
                filename_width = MAX_TORRENT_CONTENT_FILENAME_WIDTH - depth_offset - file_node_offset - dir_node_offset
            ...
            def keypress(self, size, key):
                ...
                if not self.expanded:
                    ...
                    self.get_node().content.remove_collapsed_dir(self.get_node().get_value())
                return key
        ...
        class DirectoryWidget(FlagFileWidget):
            ...
            def __init__(self, node):
                super(TorrentWindow.ContentDisplay.DirectoryWidget, self).__init__(node)
                self.expanded = not (self.get_node().get_value() in self.get_node().content.get_collapsed_dirs())
        ...
        class FileNode(uw.TreeNode):
            ...
            def load_parent(self):
                ...
                parent = TorrentWindow.ContentDisplay.DirectoryNode(self.content, parent_name)
                parent.set_child_node(self.get_key(), self)
                return parent

            def load_widget(self):
                return TorrentWindow.ContentDisplay.FileTreeWidget(self)

        class EmptyNode(uw.TreeNode):
            def load_widget(self):
                return TorrentWindow.ContentDisplay.EmptyWidget(self)
        ...
        class DirectoryNode(uw.ParentNode):
            ...
            def load_child_keys(self):
                ...
                if len(keys) == 0:
                    ...
                    self._children[None] = TorrentWindow.ContentDisplay.EmptyNode(self,
                                                                                  parent=self,
                                                                                  key=None,
                                                                                  depth=depth)
                    keys = [None]
            ...
            def load_child_node(self, key):
                ...
                if key is None:
                    return TorrentWindow.ContentDisplay.EmptyNode(None)
                else:
                    # TODO: rewrite
                    path = os.path.join(self.get_value(), key)
                    if index < self.dir_count:
                        return TorrentWindow.ContentDisplay.DirectoryNode(self.content, path, parent=self)
                    else:
                        # TODO: rewrite
                        path = os.path.join(self.get_value(), key)
                        return TorrentWindow.ContentDisplay.FileNode(self.content, path, parent=self)

            def load_widget(self):
                return TorrentWindow.ContentDisplay.DirectoryWidget(self)
